...
Attestcoin Protocol shaped the design. The README covers what PayGo is and how to run it; this
...
The Attestcoin Protocol closes that gap with inclusion proofs: a Creditcoin contract can verify
that a given Ethereum transaction exists in a block whose header Creditcoin has attested. That
primitive is *additive only*: it can establish that something happened, never that something did
not. There is no proof of non-inclusion, and no amount of engineering produces one.
...
PayGoEscrow.settle(heights[], txs[], merkleProofs[], sharedContinuityProof)
  ...
  2. VERIFIER(0x…0FD2).verifyAndEmit(batch)                        # one call, up to the 10 txs allowed
...
These are not defensive boilerplate. Each corresponds to a property of the Attestcoin Protocol's
precompiles, verified empirically against CC3 testnet before the contracts were written:
...
`EPOCH = 1000` blocks is the spacing of the Attestcoin Protocol's continuity checkpoints, so a
proof that stays inside one window carries the shortest continuity chain. Every PayGo deadline is
therefore quantised onto that grid (`firstDeadline % EPOCH == 0` and `interval % EPOCH == 0`) so
payments belonging to *different* orders land in the same window and can be proven together.
...
tokenised asset is the one that was listed. That is the gap every real-world-asset design has to face.
...
`chainKey` is the Attestcoin Protocol's identifier for the source chain and is **not** an EVM chain
id. Sepolia is `1` on CC3 testnet, and the mapping differs per environment. It is a constant per
deployment, never a user-supplied parameter.
